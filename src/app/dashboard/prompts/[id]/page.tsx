...
import type { SourceType, PromptCategory } from "@/lib/types";
...
export default async function PromptDetailPage({
  ...
  const [sources, projects, files, tags, relatedPrompts, similarPrompts] =
    await Promise.all([
      getAllSources(),
      getAllProjects(),
      getPromptFiles(prompt.id),
      getPromptTags(prompt.id),
      getRelatedPrompts(prompt.id, 5),
      getSimilarPrompts(prompt.id, 5),
    ]);

  const source = sources.find((s) => s.id === prompt.sourceId);
  const project = projects.find((p) => p.id === prompt.projectId);
  const session = prompt.sessionId
    ? await getSessionById(prompt.sessionId)
    : null;
